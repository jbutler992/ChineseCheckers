...
function evaluation(board, depth, count){
	...
	for(var i = 0; i < possibleMoves.length; i++){
        ...
        if(startRow - endRow != 0){
            if(startRow > endRow)
                var jumpedRow = startRow -1;
            else
                var jumpedRow = startRow +1;
        
            possibleBoard[jumpedRow][startColumn].isEmpty = true;
			//If not at final depth recursively search
			if(count<depth){
                var newCount = count+1;
                posNextMoves[i]=evaluation(possibleBoard,depth,newCount);
				
			}
            possibleBoard[jumpedRow][startColumn].isEmpty = false;
        }
        //Left Right move
        if(startColumn - endColumn != 0){
            ...
            possibleBoard[startRow][jumpedColumn].isEmpty = true;
            //If not at final depth recursively search
			if(count < depth){
                var newCount = count+1;
                posNextMoves[i]=evaluation(possibleBoard,depth,newCount);
			}
            possibleBoard[startRow][jumpedColumn].isEmpty = false;
        }
    ...
    //If even look for lowest
	else if(count%2==0){
       for(var i = 0; i < possibleMoves.length; i++){
           //console.log("entered odd:::"+posNextMoves[i]+":::"+lowest);
           if(posNextMoves[i] < lowest){
               lowest = posNextMoves[i];
               //console.log("lowest  Set to: "+lowest);
           }
        }
        return lowest;	
	}
    //If odd look for highest
	else{
        for(var i = 0; i < possibleMoves.length; i++){
            if(posNextMoves[i] > highest){
                highest = posNextMoves[i];
                //console.log("highest Set to: "+highest);
           }
        }
        return highest;	
	}
...
function makeMoves(){
	...
	var moveToTake = evaluation(initialBoard, 4, count);
	//Take move
